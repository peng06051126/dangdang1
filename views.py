...
from dangdangapp.models import TBooks, TSort, TAddress, TOrder, OrderItem, TConfirmString, TUser, City
...
def make_confirm_string(new_user):
    ...
    print(new_user.id)
    TConfirmString.objects.create(code=code,user_id=new_user.id)
    return code
...
#订单页模块
def indent(request):
    user = request.GET.get('user')#用户状态
    cart = request.GET.get('cart')#购物车中是否有值
    ...
    if user:
        ...
        request.session["jiesuan"]="jiesuan"
        ...
        username = request.COOKIES.get("username")
        return render(request,'dangdangapp/indent.html',{'addr':addr,'cart':car,'username':username})
    # return redirect('dangdangapp:login')
...
def sj(request):
    id = request.GET.get('id')
    m = City.objects.filter(pid=id)
    def country(c):
        if isinstance(c,City):
            return {'name':c.cityname,'id':str(c.cid)}
    return JsonResponse(list(m),safe=False,json_dumps_params={'default':country})
